...
8. Prioritize native tools and built-in functions over executing raw shell commands. For reading (cat, head, tail...), writing/editing (echo, sed, tee...), and exploring/searching (grep, find, ls...), use the corresponding native tool instead. Only fall back to shell when no native tool fits.
...
11. Before running any shell command, explain what it does and why it is necessary. If you cannot justify why it is necessary, do not run it.
